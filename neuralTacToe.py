#this program uses neural networks to predict moves for an x's and o's game
...
numberO = 0
for i in range(0,100000): #this is the number of times you want to train the player with random numbers
...
for i in finalList:
	...
	if len(i[3])==2:
		...
		ffom.append(prep3)
		lfom.append(i[3][1])
clf1 = MLPClassifier(solver='lbfgs', alpha=1e-3,hidden_layer_sizes=(2, 2), random_state=1)
clf1 = clf1.fit(np.reshape(ffm,(-1,1)),np.reshape(lfm,(-1,1)).ravel())
clf2 = MLPClassifier(solver='lbfgs', alpha=1e-3,hidden_layer_sizes=(2, 2), random_state=1)
clf2 = clf2.fit(fsm,np.reshape(lsm,(-1,1)).ravel())
clf3 = MLPClassifier(solver='lbfgs', alpha=1e-3,hidden_layer_sizes=(2, 2), random_state=1)
clf3 = clf3.fit(ftm,np.reshape(ltm,(-1,1)).ravel())
clf4 = MLPClassifier(solver='lbfgs', alpha=1e-3,hidden_layer_sizes=(2, 2), random_state=1)
clf4 = clf4.fit(ffom,np.reshape(lfom,(-1,1)).ravel())
if len(lfom)==1:
	clf4 = clf4.fit(ffom,np.reshape(lfom,(-1,1)).ravel())
